{ fetchNimble, ... }:

map fetchNimble [
  # Begin auto generated input
  
    {
      pname = "flatty";
      version = "0.3.4";
      hash = "sha256-ZmhjehmEJHm5qNlsGQvyYLajUdwhWt1+AtRppRrNtgA=";
    }
    
    {
      pname = "redis";
      version = "0.3.0";
      hash = "sha256-4gBsKJXIsYr7lUrsmJOu4aF0rLi/BnxCk0UqmsDNQlg=";
    }
    
    {
      pname = "redpool";
      version = "0.1.0";
      hash = "sha256-Rq+dFUC9MZMfGCZmBVSPs5S47N7JSONswBYqy6ErsIM=";
    }
    
    {
      pname = "ws";
      version = "0.5.0";
      hash = "sha256-3wVi6CjMTjcc5MJEozJN6W3TuYDb53w2MDCsv6lMH0k=";
    }
    
    {
      pname = "dotenv";
      version = "2.0.1";
      hash = "sha256-JH+RbloZtTj7RJsGuNUuiZ7fl+twPdgh2OVMHO/uyCM=";
    }
    
    {
      pname = "karax";
      version = "1.3.0";
      hash = "sha256-TPwQSqi7ciR6QMklWi/bOJnyzCAT1lZyxW4DTiWTALo=";
    }
    
    {
      pname = "asynctools";
      version = "0.1.1";
      hash = "sha256-4D8K8Zyr5aaKHM0qNVaPr9kUWhJUMLqWK60fZ8QEEJg=";
    }
    
    {
      pname = "httpbeast";
      version = "0.4.1";
      hash = "sha256-8ncCj94UeirSevgZP717NiNtecDyH5jHky+QId31IvQ=";
    }
    
    {
      pname = "jester";
      version = "0.5.0";
      hash = "sha256-rjbf7r5lSJZkkAZZ7qOITanyOx2o94Rx9NCXLyl7UC8=";
    }
    
    {
      pname = "jsony";
      version = "1.1.5";
      hash = "sha256-alkqZ3Q8+BzDDZo2hR3KRfzGrGhHQ/gDhxVD2TWOQJw=";
    }
    
    {
      pname = "sass";
      version = "0.3.0";
      hash = "sha256-0eXVZR6a/Ap39hyvtrJ6WYDnL0YFGQSUHHleAUGE9c8=";
    }
    
    {
      pname = "zippy";
      version = "0.10.7";
      hash = "sha256-RtEROjyxRO5opQe2045CQ3jzz72dAv03jYhMdnUMY2Y=";
    }
    
    {
      pname = "packedjson";
      version = "0.2.2";
      hash = "sha256-veQIFTkdECd7gS7SCJcxDdhZCUsIO11YHOwcgYrNAow=";
    }
    
    {
      pname = "nimcrypto";
      version = "0.6.0";
      hash = "sha256-xTzrkryW2TRbPHri3uCYugXTrA3ozO4bcqWmZdarrbY=";
    }
    
    {
      pname = "supersnappy";
      version = "2.1.3";
      hash = "sha256-s44dUPseQGfU4YwQ/gjN3jqnrSa/b1ikbpDPYnoUEAY=";
    }
    
  # End auto generated input
]
  
